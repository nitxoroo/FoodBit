...
const SignInPartner = () => {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
      setLoading(true)
      const res = await axios.post(`${serverUrl}/api/auth/partner/signIn`, { email, password }, { withCredentials: true })
      dispatch(setUserData(res.data))
      toast.success("Restaurant partner signed in successfully!")
      navigate("/")
    } catch (error) {
      console.log(error)
  ...
  return (
    <div className='min-h-screen flex items-center justify-center p-4' style={{ backgroundColor: bgColor }}>
      <div className={`bg-white rounded-xl shadow-lg w-full max-w-md p-8 border border-[${borderColor}] flex flex-col gap-4`}>
        ...
        <p>Welcome back! Sign in to your restaurant partner account.</p>
        ...
        <button onClick={handleSignIn} disabled={loading} className={`w-full bg-[#ff4d2d] text-white p-2 rounded-lg transition-colors cursor-pointer hover:bg-[#e64323] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2`}>
          {loading ? (
            <>
        ...
        <p className='text-center'>Need a restaurant partner account? <span className='text-orange-500 hover:underline cursor-pointer' onClick={() => navigate("/partner/signUp")}>Sign Up</span></p>
